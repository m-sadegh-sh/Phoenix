namespace Phoenix.WPF.CustomControls {
    ...
    public class WindowBase : Window {
        ...
        private readonly BackgroundWorker _removeWorker;
        ...
        public WindowBase(bool useBlankWindow = false, bool changeTrackingEnabled = true, bool visibleCloseButton = true) {
            ...
            _removeWorker = new BackgroundWorker {WorkerReportsProgress = true};
            _removeWorker.DoWork += RemoveWorkerDoWork;
            _removeWorker.ProgressChanged += RemoveWorkerProgressChanged;
            _removeWorker.RunWorkerCompleted += RemoveWorkerRunWorkerCompleted;

            _saveWorker = new BackgroundWorker {WorkerReportsProgress = true};
            _saveWorker.DoWork += SaveWorkerDoWork;
            _saveWorker.ProgressChanged += SaveWorkerProgressChanged;
            _saveWorker.RunWorkerCompleted += SaveWorkerRunWorkerCompleted;
        ...
        protected void TryToRemove(object argument = null) {
            if (!_removeWorker.IsBusy)
                _removeWorker.RunWorkerAsync(argument);
        }

        protected void TryToSave(object argument = null) {
            if (!_saveWorker.IsBusy)
                _saveWorker.RunWorkerAsync(argument);
        }
